import APIService from '../../../../services/map/amap/MapAPI'
...
class PlaceSearcher {
    ...
    search(polygon, keyword, keys = ["MviPFAcx5I6f1FkRQlq6iTxc"]) {
        return new Promise((resolve) => {
            let bounds = this._segmentBound(polygon, 10) // 将区域切分为100个
            async.mapLimit(bounds, 10, (bound, callback) => {
                this._placeSerarch(bound, keyword, keys, callback)
            }, (err, result) => {
                let poiDict = {} // key: poi的uid  value: poi的具体内容
                let successfulBounds = []
                let zeroBounds = []
                result.forEach(region => {
                    this._deDuplicates(poiDict, region.poiList)
